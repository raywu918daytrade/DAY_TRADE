...
# train/test/上線推論三邊用同一套規則，可以真的部署。
#
# 2026-07-25：top_n改成300後（見上面TOP_N的說明），重新用
# strategy/mkt/experiments/walk_forward_atr5_threshold.py 對p90(0.00748，
# 沿用舊值)/p95(0.00856)/p97(0.01000)/p99(0.01318)四個候選門檻各自跑一次
# 5窗口45天walk-forward比precision，門檻越高precision越好（p99在0.5~0.7
# 三個模型門檻都最好，p97在0.8最好，62.05% vs p99的60.05%），但樣本量代價
# 很大（過濾前353萬筆 → p90剩23.9萬 → p97剩9.5萬 → p99只剩3萬），p99在高
# 模型門檻(0.8)時單一窗口的候選數常常只有個位數，precision數字統計上不
# 穩定。權衡precision跟每日訊號量後選p97，改為正式設定。
#
# 門檻是「全體樣本atr5（top_n=300population）的p97分位數」，用
# strategy/mkt/experiments/atr5_pr_check.py::run_absolute_uniform() 對截至
# 2026-07-25的資料算出來的固定數字，不是每次訓練動態重算——之後資料分布
# 如果有明顯變化，要記得回來重新驗證這個數字合不合適，不要無限期沿用。
