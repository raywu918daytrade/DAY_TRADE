...
def run(start_date: str, end_date: str, max_rows: int = 2000) -> pd.DataFrame:
    ...
    print(f"母體: tick400；決策窗 {SESSION_START[0]:02d}:{SESSION_START[1]:02d}"
          f"～{SESSION_END[0]:02d}:{SESSION_END[1]:02d}", flush=True)
    print(
    ...
    for j, r in enumerate(ev.itertuples(index=False), start=1):
        ...
        if j % 400 == 0 or j == len(ev):
            print(
                f"  [tick] {j}/{len(ev)} cache_days={len(cache)} "
    ...
    print(
        f"\nmean(net|M1上)={mean_up:.4f}  mean(net|M1下)={mean_dn:.4f}  "
        f"差={mean_up - mean_dn:.4f}",
        flush=True,
    )
    print(
        f"解讀: 同向率≈50% 或兩側 mean(net)接近 → 幾乎脫鉤。"
