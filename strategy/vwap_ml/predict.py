...
def _direction_probas(model, df: pd.DataFrame):
    """把3分類機率（回歸=0/無訊號=1/延續=2）轉成「做多」「做空」機率：

        trigger_side="upper"（z>+STD_MULT，價格在VWAP之上）：
            延續(class=2) → 做多；回歸(class=0) → 做空
        trigger_side="lower"（z<-STD_MULT，價格在VWAP之下）：
            回歸(class=0) → 做多；延續(class=2) → 做空
    ...
    p_up = np.where(is_upper, p_continue, p_revert)
    p_down = np.where(is_upper, p_revert, p_continue)
    return p_up, p_down
