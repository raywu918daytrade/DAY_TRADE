...
build_poc.build() → build_m3_m5_rolling.build() → build_m3_m5_std.build()。
...
tick更新失敗不中斷整支流程，但 volume_profile/poc 都是從 tick 算出來的，
tick沒更新就沒新資料可增量，所以一併跳過（m3/m5 rolling重建只依賴db/m1，
跟tick無關，不受影響）。各支腳本仍可獨立執行，這支只是串起來方便本機一
次跑完。
...
if __name__ == "__main__":
    ...
    if tick_ok:
        ...
        print("=== 跳過 Volume Profile / POC（tick 未更新）===")
